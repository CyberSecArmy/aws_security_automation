...
import boto3									# Required modules to import
import boto3.session
import datetime
import sys

dev = boto3.session.Session(profile_name=‘non-production’)		        # creating session and credential validation

'''
NOTE: In the above code line, 'non-production' is the name of the profile being mentioned in the 'credentials' file under which,
...
start = end - datetime.timedelta(hours=2)					# start time (fetching last 2 hrs. logs, max. 3 hrs. logs could be fetched)

response = client.get_sampled_requests(					        # making api call to 'get_sampled_requests' method to get the required results
    WebAclId='a017abcd-2aa6-12d7-9c1d-123f31447abc7',				# WebAclId in question
...
sys.stdout=open("aws_waf_np_logs.txt","a")
print ("**************************************************************")
